...
const SideNavbar = () => {
  ...
  return (
    <div className="lg:w-[25%] sticky border-r-2 border-gray-500 overflow-hidden -top-1 sm:flex flex-col justify-between h-screen md:w-[32%] text-nowrap text-white py-3 px-5 hidden  w-0 sm:w-[150px] bg-black">
          ...
          {SidebarOptions.map((option, idx) => {
            return (
              <Link
                ...
                className="flex text-2xl w-full gap-4 hover:bg-[#da55e69f] duration-300 transition-all cursor-pointer py-3 px-2 justify-center md:justify-start items-center rounded-4xl"
              >
      ...
      <Link
        to={"/settings"}
        className="flex mb-2 cursor-pointer flex-col md:flex-row gap-3 w-full justify-start items-center"
      >
        <IoSettings size={"25"} />
      </Link>
    </div>
